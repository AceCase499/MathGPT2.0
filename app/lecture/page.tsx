...
export default function jsChat(){
  ...
  const [LoadedSession, setLoadedSession] = useState('NEW SESSION')
...
`;
  ...
  function loadConvo(indexx){
    let newChat = ChatArchive[indexx].chat
    setChatStream(newChat)
    setLoadedSession(ChatArchive[indexx].name)
  }

  function deleteConvo(indexx){
    let tempArc = ChatArchive

    let dialogue = "Delete this chat session?";
    if(LoadedSession === tempArc[indexx].name){
      dialogue += "\n(This will end your current session.)"
    }
    if (confirm(dialogue) == true) {
      tggUpdating(true)
      let newArchive = ChatArchive
      newArchive.splice(indexx, 1)
      setChatArchive(newArchive)
  ...
  function renameConvo(indexx){
    let newSessionName = prompt(
      ...
      `${LoadedSession} - ${new Date().toDateString()}` //placeholder
    )

    if (newSessionName != null && newSessionName.trim()){
      tggUpdating(true)
      let tempConvo = ChatArchive
      tempConvo[indexx].name = newSessionName
      setChatArchive(tempConvo)
  ...
  return (
    <div style={{ fontFamily: "'Permanent Marker', cursive"}}>
      ...
      <div className="splitsub left bg-amber-200 text-xl h-full">
        {/* This div holds the left panel */}
        <h1 className="text-blue-400 px-3 pt-9 font-extrabold">Previous Sessions:</h1>
        {/*Previous Topics loaded in from database are placed here*/}
        {!updating && ChatArchive.map((cht, index) => (
            <div id={index.toString()} key={index} style={{padding: 4, borderRadius: 20}} className='parent flex cursor-pointer bg-transparent hover:bg-amber-400'>
      ...
      <div className="splitmain right text-lg">
        ...
        <center>
          ...
          <div ref={chatContainer} style={{boxShadow: "0 0 30px rgb(230, 230, 230)", flex: 1, overflowY: "scroll", height: "55vh"}}> {/* //This div streams the response in real time, couldn't finish it in time */}
