...
import { subtopicsGeometry } from './subtopics';

const topics = [
    { id: nanoid(), name: 'Geometry', image: '/images/geometry.png', color: 'bg-pink-100', progress: 22 }
];
...
export default function SubtopicAlgebra2Page() {
    ...
    const [subTopicsState, setSubTopicsState] = useState(subtopicsGeometry);
    ...
    const toggleSubtopicFavorite = (id: string) => {
        ...
        const sortedSubtopics = [...updatedSubtopics].sort((a, b) => {
            ...
            return subtopicsGeometry.findIndex(sub => sub.id === a.id) - subtopicsGeometry.findIndex(sub => sub.id === b.id);
        });
