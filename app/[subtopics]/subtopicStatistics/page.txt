...
import { subtopicsStatistics } from './subtopics';

const topics = [
    { id: nanoid(), name: 'Statistics', image: '/images/statistics.png', color: 'bg-pink-100', progress: 22 }
];
...
export default function SubtopicAlgebra2Page() {
    ...
    const [subTopicsState, setSubTopicsState] = useState(subtopicsStatistics);
    ...
    const toggleSubtopicFavorite = (id: string) => {
        ...
        const sortedSubtopics = [...updatedSubtopics].sort((a, b) => {
            ...
            return subtopicsStatistics.findIndex(sub => sub.id === a.id) - subtopicsStatistics.findIndex(sub => sub.id === b.id);
        });
